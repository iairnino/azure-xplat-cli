...
exports.init = function (cli) {
  ...
  var vnetGatewayIpConfig = vnetGateway.category('ip-config')
    .description($('Commands to manage Virtual Network Gateway VIP configuration'));

  vnetGatewayIpConfig.command('add [resource-group] [vnet-gateway-name] [name]')
    .description($('Add an IP configuration to Virtual Network Gateway'))
    .usage('[options] <resource-group> <vnet-gateway-name> [name]')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-w, --vnet-gateway-name <vnet-gateway-name>', $('the name of the virtual network gateway'))
    ...
    .option('-e, --subnet-name <subnet-name>', $('the subnet name'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, vnetGatewayName, name, options, _) {
      resourceGroup = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroup, _);
      vnetGatewayName = cli.interaction.promptIfNotGiven($('VPN Gateway name: '), vnetGatewayName, _);
      ...
      vpnGateway.addIpConfig(resourceGroup, vnetGatewayName, name, options, _);
    });

  vnetGatewayIpConfig.command('remove [resource-group] [vnet-gateway-name] [name]')
    .description($('Remove an IP configuration to Virtual Network Gateway'))
    .usage('[options] <resource-group> <vnet-gateway-name> [name]')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-w, --vnet-gateway-name <vnet-gateway-name>', $('the name of the virtual network gateway'))
    .option('-n, --name <name>', $('the name of the ip configuration'))
    ...
    .execute(function (resourceGroup, vnetGatewayName, name, options, _) {
      ...
      var networkResourceProviderClient = getNetworkResourceProviderClient(options);
      var vpnGateway = new VirtualNetworkGateway(cli, networkResourceProviderClient);
      vpnGateway.removeIpConfig(resourceGroup, vnetGatewayName, name, options, _);
    });
