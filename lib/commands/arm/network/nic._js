...
__.extend(Nic.prototype, {
  ...
  list: function (resourceGroupName, params, _) {
    ...
    self.interaction.formatOutput(nics.networkInterfaces, function (data) {
      if (data.length === 0) {
        self.output.info($('No network interfaces found'));
      } else {
  ...
  addInboundRule: function (resourceGroupName, nicName, params, _) {
    this._updateInboundRule(resourceGroupName, nicName, params, true, _);
  },
